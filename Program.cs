...
{   
    int numberRow=0;
        ...
        for( int i = 0; i < array.GetLength(0); i++) // внутренний цикл, в котором ищем сумму элементов  построчно
    {    
        int sum=0;
        int minSum=0;
        for( int j = 0; j < array.GetLength(0); j++)
        {
         ...
         sum=0;                            // сумму обнуляем, она не нужна для решения дальше
    }
    Console.WriteLine($"The row with the smallest sum of elements is  ");   
}
int[,] myArray=CreateRandom2dArray();
Show2dArray(myArray);
